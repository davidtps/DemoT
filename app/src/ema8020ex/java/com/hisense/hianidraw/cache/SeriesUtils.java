...
public class SeriesUtils {
    ...
    public static final int EPOS_MIN_HEIGHT = 149;//px
    public static final int EPOS_MIN_WIDTH = 0;//px
    ...
    public static final boolean OLED_SERIES = true;//true  no usb pic and built-in pic function
    public static final int UPDATE_VIDEO_FILE_MAX_SIZE = 400;//unit is M
    public static final long MPERIODTIME_JSON = 77100L;
    public static final int EPOS_LEFT_MARGIN = 0;//px
    public static final int DELAY_START_STOREMODE = 4000;//ms
    public static final boolean EPOS_PLAY_REPEAT = true;//epos play repeat
    public static boolean DEFAULT_PLAY_SOURECE_BUILT_IN_VIDEO = true;//  没有选择任何资源时，true 默认播放内置视频,false 默认播放内置图片
    ...
    public static void initSpecialSeriesFunction() {
        ...
        if (epos_position == 0 || epos_position == 2) {
            ConstantConfig.EPOS_LAYOUT_POSITION = 1;//top
            PreferenceUtils.getInstance().save(ConstantConfig.SHAREPREF_EPOS_POSITION, 1);//top
        }

        if (malllogo_position == 1 || malllogo_position == 3) {
            ConstantConfig.MALL_LOGO_POSITION = 2;//right
            PreferenceUtils.getInstance().save(ConstantConfig.SHAREPREF_MALL_LOGO_POSITION, 2);//right
        }
    }
    ...
    public static boolean isSupportNewAqPq() {
        switch (ConstantConfig.mProductName) {
            case "50M5306EXT":////EM Toshiba
            case "55M5306EXT":
            case "65M5306EXT":
            case "75M5306EXT": {
                isSupportNewAqPqJar = false;
                break;
            }
                ...
                switch (ConstantConfig.mProductName) {
                    ...
                    case "75M5306EXT": {
                        if (ConstantConfig.mBuildSoftVersion[0].equals("V1000")) {
                            inputStream = assetManager.open("epos_horizontal_m5306ext.json");
                        } else {
                            inputStream = assetManager.open("epos_horizontal_m5306ext.json");
                        }
                        break;
                    ...
                    case "TT65M540UW":
                    case "TT55M540UW": {
                        inputStream = assetManager.open("epos_horizontal_ttm540uw.json");
                        break;
                    }
                    default: {
                        inputStream = assetManager.open("epos_horizontal_m5306ext.json");
